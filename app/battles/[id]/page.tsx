import {routeRequirements,routeStages} from "../../archive/campaign-routes";
import {notFound} from "next/navigation";
import {Footer,Header} from "../../components";
import {battles,officerById} from "../../data";
import {stageGuides,weaponGuides,dossiers,manual,edition} from "../../archive/dw8xl";
...
export default async function BattlePage({params}:{params:Promise<{id:string}>}){
 ...
 const stage=[...stageGuides,...routeStages].find(b=>b.id===id);const rewards=weaponGuides.filter(w=>w.rare.stageId===id);
 const officerIds=[...new Set([...(stage?.officerIds??[]),...Object.entries(dossiers).filter(([,p])=>p.battles.includes(id)).map(([id])=>id)])];
 const requirements=routeRequirements(id);
 ...
 <h2>Connected officers</h2><p>Game portrayal and guide associations; this is not a complete playable roster or a historical order of battle.</p>{officerIds.length===0&&<p>Officer associations for this scenario await research.</p>}<div className="link-list">{officerIds.map(id=><a key={id} href={`/officers/${id}`}>{officerById(id)?.name} →</a>)}</div>
 ...
 {requirements.map(({route,step})=><details className="spoiler-panel" key={route.id}><summary>Reveal {route.name} route requirement</summary><p>{step.goal}</p><p>{step.reward}</p><a href={`/battles#${route.id}-route`}>See the full prerequisite checklist →</a><SourceLinks sources={route.sources}/></details>)}
 ...
 {!requirements.length&&!rewards.length&&<p>Detailed objectives for this battle are still open for research.</p>}
 </div><aside className="provenance"><h2>Source trail</h2><SourceLinks sources={[manual,...rewards.map(w=>w.rare.source),...requirements.flatMap(({route})=>route.sources)]}/><a href={`/contribute?recordType=battle&recordId=${id}`}>Suggest a correction →</a></aside></section><Footer/></main>
}
