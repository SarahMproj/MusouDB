...
export default async function Coverage(){
 ...
 return <main><Header active="officers"/><section className="archive-hero coverage-hero"><p className="eyebrow">EDITORIAL COVERAGE · DW8XL CE</p><h1>{rows.length} officers. Visible research gaps.</h1><p>A research profile includes historical context, game portrayal and a linked weapon guide. It does not imply that every move, unlock or appearance has been verified.</p><div className="coverage-stats"><div><strong>{researched}</strong><span>Research profiles</span></div><div><strong>{weaponGuides.length}</strong><span>Rare weapon guides</span></div><div><strong>{published}</strong><span>Officers with published research</span></div><div><strong>{rows.length-researched}</strong><span>Seed profiles to expand</span></div></div></section>
 ...
 <p>Publisher documentation supports the general systems. Rare conditions and playstyles are attributed to community guides and await MusouDB replay verification. All five campaigns have branch checklists. Full moveset tables, detailed route walkthroughs and in-game replay checks remain open.</p>
