...
import {OfficerPortrait} from "../../officer-portrait";
...
export default async function OfficerPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main><Header active="officers"/>
    <section className={`detail-hero officer-hero ${o.style}`}><figure className="officer-hero-art"><OfficerPortrait id={o.id} name={o.name} mark={o.mark} hero/><figcaption>MusouDB original · AI-generated interpretation</figcaption></figure><div className="officer-hero-copy"><p className="eyebrow"><span/>{kingdomFor(o)} OFFICER · {o.sourceStatus}</p><h1>{o.name}</h1><p className="detail-alias">{o.title} · {o.alias}</p><p>{o.summary}</p><SaveButton kind="officers" id={o.id} label={o.name}/></div></section>
    <section className="detail-body"><div className="detail-main">
      ...
      {dossier&&!detail.gameplay&&<section className="officer-narrative"><h2>Combat research</h2><p>The signature EX weapon is sourced. Individual attack strings, playstyle notes and rare weapon objectives still need verification for this officer.</p></section>}
    ...
    </div><aside className="provenance"><p className="eyebrow"><span/>PROVENANCE</p><h3>{profile?"DW8XL CE research profile":"Community-authored seed"}</h3><p>{profile?"Historical overviews, publisher information and community gameplay observations are identified separately. Core profiles do not imply complete combat research. Any community gameplay notes remain untested by MusouDB.":"This initial record is open for sourced expansion by the MusouDB community."}</p><dl><div><dt>Status</dt><dd>{o.sourceStatus}</dd></div><div><dt>Spoiler class</dt><dd>{detail.spoiler||detail.unlock?"Separated":"Spoiler-safe"}</dd></div><div><dt>Imagery</dt><dd>AI-generated original portrait. An artistic interpretation, not a historical likeness or official game artwork.</dd></div></dl>{sourceRows.length>0&&<div className="citation-stack">{sourceRows.map((c,i)=><a href={c.url} target="_blank" rel="noreferrer" key={i}><span>{c.kind}</span>{c.label} ↗{c.fields.length>0&&<small>Supports: {c.fields.join(", ")}</small>}</a>)}</div>}<a href={`/contribute?recordType=officer&recordId=${o.id}`}>Suggest a correction ↗</a></aside></section><Footer/></main>
}
