...
export default async function OfficerPage({params}:{params:Promise<{id:string}>}){
  ...
  const profile=officerProfiles[id];const dossier=dossiers[id];const weaponGuide=weaponGuides.find(w=>w.officerId===id);
  ...
  return <main><Header active="officers"/>
    ...
    <section className="detail-body"><div className="detail-main">
      {unavailable&&<p role="status">Community updates are temporarily unavailable. Showing the seed record; please try again later.</p>}{dossier&&<p className="edition-label">Gameplay scope: <a href="/games/dw8xl">{edition}</a></p>}<h2>Record</h2><dl className="fact-grid"><div><dt>Kingdom</dt><dd>{kingdomFor(o)}</dd></div><div><dt>Signature weapon</dt><dd>{weaponGuide&&o.weapon===weaponGuide.name?<a href={`/weapons/${weaponGuide.id}`}>{o.weapon} →</a>:o.weapon}</dd></div><div><dt>Role</dt><dd>{profile?.role||o.title}</dd></div><div><dt>Editorial coverage</dt><dd>{publication.approvedCount?`${publication.approvedCount} research fields published`:profile?"Research profile · full moveset verification open":"Seed record"}</dd></div></dl>
      {dossier&&<section className="officer-narrative"><h2>Historical context</h2><p>{dossier.history}</p><a href={dossier.historySource.url} target="_blank" rel="noreferrer">{dossier.historySource.label} ↗</a></section>}{(detail.biography||detail.gameplay)&&<section className="officer-narrative"><p className="eyebrow"><span/>IN THE CHRONICLE</p>{detail.biography&&<><h2>Game portrayal</h2><small>{publication.fields.biography?"Published contributor research":"MusouDB editorial reading of the game’s characterization"}</small>{manual?.spoilerClass!=="safe"&&!publication.fields.biography&&manual?.body?<details className="spoiler-panel"><summary>Reveal biography notes</summary><p>{detail.biography}</p></details>:<p>{detail.biography}</p>}</>}{detail.gameplay&&<><h2>How they play</h2><p>{detail.gameplay}</p>{!publication.fields.gameplay&&dossier&&<a href={combatSource.url} target="_blank" rel="noreferrer">Community playstyle analysis ↗</a>}</>}</section>}
      ...
      {weaponGuide&&<div className="link-list"><a href={`/weapons/${weaponGuide.id}`}><strong>{weaponGuide.name}</strong><small>EX notes, compatibility and rare weapon guide →</small></a></div>}{detail.unlock&&<details className="spoiler-panel"><summary>Reveal unlock conditions</summary><p>{detail.unlock}</p></details>}
