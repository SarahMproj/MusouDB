...
export default async function OfficerPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main><Header active="officers"/>
    ...
    </div><aside className="provenance"><p className="eyebrow"><span/>PROVENANCE</p><h3>{profile?"DW8XL CE research profile":"Community-authored seed"}</h3><p>{profile?"Historical overviews, publisher information and community gameplay observations are identified separately. Core profiles do not imply complete combat research. Any community gameplay notes remain untested by MusouDB.":"This initial record is open for sourced expansion by the MusouDB community."}</p><dl><div><dt>Status</dt><dd>{o.sourceStatus}</dd></div><div><dt>Spoiler class</dt><dd>{detail.spoiler||detail.unlock?"Separated":"Spoiler-safe"}</dd></div><div><dt>Imagery</dt><dd>Original geometric placeholder</dd></div></dl>{sourceRows.length>0&&<div className="citation-stack">{sourceRows.map((c,i)=><a href={c.url} target="_blank" rel="noreferrer" key={i}><span>{c.kind}</span>{c.label} ↗{c.fields.length>0&&<small>Supports: {c.fields.join(", ")}</small>}</a>)}</div>}<a href={`/contribute?recordType=officer&recordId=${o.id}`}>Suggest a correction ↗</a></aside></section><Footer/></main>
}
